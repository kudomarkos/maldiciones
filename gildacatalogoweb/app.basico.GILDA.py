...
PORT = 8019
DB_PATH = os.path.join(os.path.dirname(__file__), "gilda.sqlite")
TABLE = "GILDATABLE"
...
class Handler(http.server.BaseHTTPRequestHandler):
    ...
    def do_GET(self):
        ...
        if path == "/":
            # serve index.html
            try:
                with open("catalogadorGILDA.html", "rb") as f:
                    content = f.read()
                self._send(200, "text/html; charset=utf-8", content)
            except FileNotFoundError:
                self._send(404, "text/plain; charset=utf-8", b"catalogadorGILDA not found")
            return
    ...
    def do_POST(self):
        ...
        if path == "/api/copy_fields":
            ...
            conn = self._db()
            try:
                cur = conn.execute("SELECT TITULO, PP, TT, DIALOGO FROM " + TABLE + " WHERE ROWID = ?", (src,))
                row = cur.fetchone()
                if not row:
                    self._send(404, "application/json; charset=utf-8", json.dumps({"ok":False,"error":"source not found"}).encode("utf-8"))
                    return
                conn.execute("UPDATE " + TABLE + " SET TITULO = ?, PP = ?, TT = ?, DIALOGO = ? WHERE ROWID = ?",
