...
def update_cell():
    ...
    try:
        ...
        cur = db.execute("SELECT ID, PUBLICACION, ANIO, MITIPO, TITULO, PP, TT, DIALOGO, ROWID FROM ZZODS23ENERO WHERE ID = ?", (row_id,))
        ...
        if not (bad(updated["TITULO"]) or bad(updated["PP"]) or bad(updated["TT"])):
            ...
            q = """
            SELECT ID, PUBLICACION, ANIO, MITIPO, TITULO, PP, TT, DIALOGO, ROWID
            FROM ZZODS23ENERO
